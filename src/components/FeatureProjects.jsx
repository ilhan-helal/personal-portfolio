...
useEffect(() => {
  ...
  ref={sectionRef}
  id="feature"
  className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 border-t border-white/10 relative overflow-hidden"
  style={{
...
>
      ...
      {Array.from({ length: typeof window !== "undefined" && window.innerWidth < 768 ? 6 : 15 }).map((_, i) => (
        <motion.div
          key={i}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="relative">
          ...
          <button
            onClick={prevSlide}
            className="absolute left-0 sm:left-0 top-1/2 -translate-y-1/2 sm:-translate-x-6 z-10 w-6 h-6 sm:w-12 sm:h-12 bg-white/10 rounded-full flex items-center justify-center border border-white/20"
          >
            <ChevronLeft className="text-yellow-400 w-4 h-4 sm:w-6 sm:h-6" />
          </button>

          <button
            onClick={nextSlide}
            className="absolute right-0 sm:right-0 top-1/2 -translate-y-1/2 sm:translate-x-6 z-10 w-6 h-6 sm:w-12 sm:h-12 bg-white/10 rounded-full flex items-center justify-center border border-white/20"
          >
            <ChevronRight className="text-yellow-400 w-4 h-4 sm:w-6 sm:h-6" />
          </button>
                ...
                <div key={project.id} className="w-full shrink-0 px-2">
                  <div className="grid md:grid-cols-2 bg-white/5 rounded-2xl overflow-hidden border border-white/10">
                    ...
                    <div className="p-6 sm:p-8 md:p-12 flex flex-col justify-between">
                      <div>
                        <h3
                          className={`text-xl sm:text-2xl md:text-3xl font-bold mb-4 bg-gradient-to-r ${projectBackgrounds[idx].accentColor} bg-clip-text text-transparent`}
                        >
                          {project.title}
                        </h3>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                        <a
                          ...
                          target="_blank"
                          className="px-5 sm:px-6 py-3 bg-white text-black rounded-full flex items-center gap-2 hover:bg-black hover:text-white transition-colors duration-300"
                        >
                          <ExternalLink size={16} />
                          Live
                        </a>

                        <a className="px-5 sm:px-6 py-3 border border-white/30 rounded-full flex items-center gap-2 text-white hover:bg-white/10 transition-colors duration-300">
                          <Github size={16} />
                          Code
                        </a>
                      </div>
